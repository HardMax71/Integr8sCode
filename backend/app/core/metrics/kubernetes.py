...
class KubernetesMetrics(BaseMetrics):
    ...
    def _create_instruments(self) -> None:
        ...
        self.pod_creations = self._meter.create_counter(
            name="pod.creations.total",
            description="Total number of pod creations",
            unit="1"
        )

        self.pod_creation_failures = self._meter.create_counter(
            name="pod.creation.failures.total",
            description="Total number of pod creation failures by reason",
            unit="1"
        )

        self.pod_creation_duration = self._meter.create_histogram(
            name="pod.creation.duration",
            description="Time taken to create pods in seconds",
            unit="s"
        )

        self.active_pod_creations = self._meter.create_up_down_counter(
            name="pod.creations.active",
            description="Number of pods currently being created",
            unit="1"
        )

        # Pod lifecycle metrics
        self.pod_phase_transitions = self._meter.create_counter(
            name="pod.phase.transitions.total",
            description="Total number of pod phase transitions",
            unit="1"
        )

        self.pod_lifetime = self._meter.create_histogram(
            name="pod.lifetime",
            description="Total lifetime of pods in seconds",
            unit="s"
        )
        ...
        self.config_maps_created = self._meter.create_counter(
            name="configmaps.created.total",
            description="Total number of ConfigMaps created",
            unit="1"
        )

        self.network_policies_created = self._meter.create_counter(
            name="networkpolicies.created.total",
            description="Total number of NetworkPolicies created",
            unit="1"
        )

        # Pod monitor metrics
        self.pod_monitor_events = self._meter.create_counter(
            name="pod.monitor.events.total",
            description="Total number of pod monitor events",
            unit="1"
        )
        ...
        self.pod_monitor_watch_errors = self._meter.create_counter(
            name="pod.monitor.watch.errors.total",
            description="Total number of pod monitor watch errors",
            unit="1"
        )
        ...
        self.pods_monitored = self._meter.create_up_down_counter(
            name="pods.monitored",
            description="Number of pods currently being monitored",
            unit="1"
        )
    ...
    def record_pod_creation_duration(self, duration_seconds: float, language: str) -> None:
        self.pod_creation_duration.record(
            duration_seconds,
    ...
    def record_k8s_network_policy_created(self, status: str) -> None:
        self.network_policies_created.add(
            1,
            attributes={"status": status}
        )
    ...
    def record_pod_monitor_reconciliation_run(self, status: str) -> None:
        self.pod_monitor_reconciliations.add(
            1,
            attributes={"status": status}
        )

    def record_pod_monitor_watch_error(self, error_type: str) -> None:
        self.pod_monitor_watch_errors.add(
            1,
    ...
    def record_pod_resource_request(self, resource_type: str, value: float, language: str) -> None:
        self.pod_resource_requests.record(
            value,
            attributes={
    ...
    def record_pod_resource_limit(self, resource_type: str, value: float, language: str) -> None:
        self.pod_resource_limits.record(
            value,
            attributes={
                "resource_type": resource_type,
                "language": language
