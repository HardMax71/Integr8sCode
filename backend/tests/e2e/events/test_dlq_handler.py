...
async def test_dlq_handler_with_retries(scope: AsyncContainer, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    calls: list[tuple[str | None, KafkaTopic, str, int]] = []

    async def _record_send_to_dlq(
        original_event: DomainEvent, original_topic: KafkaTopic, error: Exception, retry_count: int
    ) -> None:
    ...
    h = create_dlq_error_handler(p, original_topic=KafkaTopic.SAGA_EVENTS, max_retries=2, logger=_test_logger)
    e = SagaStartedEvent(
        saga_id="s",
        saga_name="n",
        execution_id="x",
        initial_event_id="i",
        metadata=EventMetadata(service_name="a", service_version="1", user_id="test"),
    )
    ...
    await h(RuntimeError("boom"), e)
    assert len(calls) == 1
    assert calls[0][1] == KafkaTopic.SAGA_EVENTS
...
async def test_immediate_dlq_handler(scope: AsyncContainer, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(p, "send_to_dlq", _record_send_to_dlq)
    h = create_immediate_dlq_handler(p, original_topic=KafkaTopic.SAGA_EVENTS, logger=_test_logger)
    e = SagaStartedEvent(
        saga_id="s2",
        saga_name="n",
        execution_id="x",
