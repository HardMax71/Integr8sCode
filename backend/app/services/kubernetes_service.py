...
class KubernetesService:
    def __init__(self):
        ...
        try:
            logging.info(
                f"KUBERNETES_CONFIG_PATH: {self.settings.KUBERNETES_CONFIG_PATH}"
            )
            logging.info(
            ...
            if os.path.exists("/var/run/secrets/kubernetes.io/serviceaccount"):
                ...
                config.load_kube_config(
                    config_file=self.settings.KUBERNETES_CONFIG_PATH
                )
                logging.info(
            ...
            if ca_cert_path and os.path.exists(ca_cert_path):
                ...
                logging.warning(
                    "Custom CA certificate not found. Using default CA bundle."
                )
    ...
    async def create_execution_pod(
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: self.v1.create_namespaced_pod(
    ...
    async def get_pod_logs(self, execution_id: str):
        ...
        try:
            ...
            logs = await loop.run_in_executor(
                None,
                lambda: self.v1.read_namespaced_pod_log(
                    name=pod_name, namespace=namespace
                ),
            )
            ...
            if e.status == 404:
                logging.error(
                    f"Pod {pod_name} not found. It might have been deleted already."
                )
            else:
            ...
            try:
                await loop.run_in_executor(
                    None,
                    lambda: self.v1.delete_namespaced_pod(
                        name=pod_name, namespace=namespace
                    ),
                )
