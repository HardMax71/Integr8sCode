"""
Integration tests for Events routes against the backend.

These tests run against the actual backend service running in Docker,
providing true end-to-end testing with:
- Real event persistence in MongoDB
- Real Kafka event publishing
- Real event querying and filtering
- Real correlation tracking
- Real aggregation pipelines
- Real event replay functionality
"""
...
class TestEventsRoutesReal:
        ...
        if response.status_code == 200:
            ...
            for event in events_response.events:
                if event.event_type:  # Some events might have been created
                    assert any(event_type in event.event_type for event_type in event_types) or len(events_response.events) == 0
    ...
    @pytest.mark.asyncio
    async def test_replay_aggregate_events_requires_admin(self, client: AsyncClient, shared_user: Dict[str, str]) -> None:
