"""
Schema Registry Manager using dataclasses-avroschema.

Handles Avro serialization with Confluent wire format for Kafka messaging.
"""
...
class SchemaRegistryManager:
    ...
    async def _ensure_schema_registered(self, event_class: type[DomainEvent]) -> int:
        """Lazily register schema and return schema ID."""
        ...
        avro_schema_dict = event_class.avro_schema_to_python()
        avro_schema_obj = schema.AvroSchema(avro_schema_dict)

        schema_id: int = await self._client.register(subject, avro_schema_obj)
        self._schema_id_cache[event_class] = schema_id
        self._id_to_class_cache[schema_id] = event_class
        self.logger.debug(f"Registered schema {event_class.__name__}: ID {schema_id}")
        return schema_id
    ...
    async def serialize_event(self, event: DomainEvent) -> bytes:
        ...
        # Ensure schema is registered (lazy registration)
        await self._ensure_schema_registered(event_class)

        # Get schema and serialize
        avro_schema_dict = event_class.avro_schema_to_python()
        avro_schema_obj = schema.AvroSchema(avro_schema_dict)

        # Prepare payload - use model_dump for dict representation
    ...
    async def deserialize_event(self, data: bytes, topic: str) -> DomainEvent:
        ...
        # Restore event_type if missing (it's a discriminator field with default)
    ...
    async def set_compatibility(self, subject: str, mode: str) -> None:
        ...
        self.logger.debug(f"Set {subject} compatibility to {mode}")

    async def initialize_schemas(self) -> None:
        """Initialize all event schemas in the registry with FORWARD compatibility."""
