"""
Integration tests for admin routes against the backend.

These tests run against the actual backend service running in Docker,
not fake/mock services. This provides true end-to-end testing with:
...
class TestAdminUsersReal:
    ...
    async def test_list_users_with_pagination(self, client: AsyncClient, shared_admin: Dict[str, str]) -> None:
        ...
        assert data["total"] >= 2  # At least our test users
...
class TestAdminEventsReal:
    ...
    async def test_browse_events(self, client: AsyncClient, shared_admin: Dict[str, str]) -> None:
        ...
        browse_payload = {
            "filters": {
                "event_types": ["user.registered", "user.logged_in"]
            },
            "skip": 0,
            "limit": 20,
            "sort_by": "timestamp",
    ...
    @pytest.mark.asyncio
    async def test_admin_user_rate_limits_and_password_reset(self, client: AsyncClient, shared_admin: Dict[str, str]) -> None:
