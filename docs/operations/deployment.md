...
| Backend API | 384 m | 2 gunicorn workers | Controlled by `WEB_CONCURRENCY` env var |
...
Core infrastructure services (MongoDB, Redis, Kafka, Zookeeper, backend, frontend) also have `restart: unless-stopped` so they recover automatically after an OOM kill or crash.
