<script lang="ts">
    ...
    import { UserFilters, UsersTable, UserFormModal, DeleteUserModal, RateLimitsModal } from '$components/admin/users';
    ...
    // User form state
    let userForm = $state<{ username: string; email: string; password: string; role: UserRole; is_active: boolean }>({
        username: '',
        email: '',
        password: '',
        role: 'user',
    ...
    async function loadUsers(): Promise<void> {
        loading = true;
        const data = unwrapOr(
            await listUsersApiV1AdminUsersGet({
                query: {
    ...
    function filterUsers(userList: UserResponse[], status: StatusFilter, advanced: AdvancedFilters): UserResponse[] {
    ...
    async function saveUser(): Promise<void> {
        ...
        if (editingUser) {
            ...
            result = await updateUserApiV1AdminUsersUserIdPut({
    ...
    async function resetRateLimits(): Promise<void> {
        ...
        unwrap(
            await resetUserRateLimitsApiV1AdminRateLimitsUserIdResetPost({
                path: { user_id: rateLimitUser.user_id },
            }),
        );
        rateLimitUsage = {};
