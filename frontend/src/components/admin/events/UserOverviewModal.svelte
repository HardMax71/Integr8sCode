        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="space-y-2 text-sm">
                    <div>
                        <span class="text-fg-muted dark:text-dark-fg-muted">User ID:</span>
                        <span class="font-mono">{overview.user.user_id}</span>
                    </div>
                    <div>
                        <span class="text-fg-muted dark:text-dark-fg-muted">Username:</span>
                    ...
                    <div>
                        <span class="text-fg-muted dark:text-dark-fg-muted">Active:</span>
                        {overview.user.is_active ? 'Yes' : 'No'}
                    </div>
                    <div>
                        <span class="text-fg-muted dark:text-dark-fg-muted">Superuser:</span>
                        {overview.user.is_superuser ? 'Yes' : 'No'}
                    </div>
                </div>
                {#if overview.rate_limit_summary}
                    <div class="mt-4">
                        ...
                        <div class="text-sm space-y-1">
                            <div>
                                <span class="text-fg-muted dark:text-dark-fg-muted">Bypass:</span>
                                {overview.rate_limit_summary.bypass_rate_limit ? 'Yes' : 'No'}
                            </div>
                            <div>
                                <span class="text-fg-muted dark:text-dark-fg-muted">Global Multiplier:</span>
                                {overview.rate_limit_summary.global_multiplier ?? 1.0}
                            </div>
                            <div>
                                <span class="text-fg-muted dark:text-dark-fg-muted">Custom Rules:</span>
                                {overview.rate_limit_summary.has_custom_limits ? 'Yes' : 'No'}
                            </div>
                        </div>
                    </div>
                {/if}
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="p-3 rounded-lg bg-green-50 dark:bg-green-900/20">
                        ...
                        <div class="text-xl font-semibold text-green-800 dark:text-green-200">
                    ...
                    <div class="p-3 rounded-lg bg-red-50 dark:bg-red-900/20">
                        ...
                        <div class="text-xl font-semibold text-red-800 dark:text-red-200">
                ...
                <div class="mt-3 text-sm text-fg-muted dark:text-dark-fg-muted">
                    Terminal Total: <span class="font-semibold text-fg-default dark:text-dark-fg-default"
                        >{overview.derived_counts.terminal_total}</span
                    >
                </div>
                <div class="mt-2 text-sm text-fg-muted dark:text-dark-fg-muted">
                    Total Events: <span class="font-semibold text-fg-default dark:text-dark-fg-default"
                        >{overview.stats.total_events}</span
                    >
                </div>
            </div>
        </div>
                ...
                <div class="space-y-1 max-h-48 overflow-auto">
                    {#each recentEvents as ev}
                        <div class="flex justify-between items-center p-2 bg-neutral-50 dark:bg-neutral-800 rounded">
                            <div class="text-sm">
                                <span class={getEventTypeColor(ev.event_type)}
                                    >{getEventTypeLabel(ev.event_type) || ev.event_type}</span
                                >
                                <span class="ml-2 font-mono text-xs text-fg-muted dark:text-dark-fg-muted"
                                    >{ev.aggregate_id || '-'}</span
                                >
                            </div>
