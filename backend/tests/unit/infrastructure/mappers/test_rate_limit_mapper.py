...
def test_user_rate_limit_mapper_roundtrip_and_dates() -> None:
    ...
    u = UserRateLimit(user_id="u1", rules=[RateLimitRule(endpoint_pattern="/x", group=EndpointGroup.API, requests=1, window_seconds=1)], notes="n")
    ...
    d["created_at"] = now.isoformat(); d["updated_at"] = (now + timedelta(seconds=1)).isoformat()
    u3 = UserRateLimitMapper.from_dict(d)
    assert u3.created_at <= u3.updated_at


def test_config_mapper_roundtrip_and_json() -> None:
    cfg = RateLimitConfig(default_rules=[RateLimitRule(endpoint_pattern="/a", group=EndpointGroup.API, requests=1, window_seconds=1)],
