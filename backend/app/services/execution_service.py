...
_DomainExecutionAdapter = TypeAdapter(DomainExecution)
    ...
    async def execute_script_idempotent(
        ...
        *,
        ...
        if idempotency_result.is_duplicate:
            ...
            return _DomainExecutionAdapter.validate_json(cached_json)
        ...
        try:
            ...
            await self.idempotency_manager.mark_completed_with_json(
                event=pseudo_event,
                cached_json=_DomainExecutionAdapter.dump_json(exec_result).decode(),
                key_strategy=KeyStrategy.CUSTOM,
                custom_key=custom_key,
            )
