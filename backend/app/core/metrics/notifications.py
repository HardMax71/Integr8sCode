...
class NotificationMetrics(BaseMetrics):
    ...
    def _create_instruments(self) -> None:
        ...
        self.notifications_failed = self._meter.create_counter(
            name="notifications.failed.total",
        ...
        self.notification_delivery_time = self._meter.create_histogram(
            ...
            description="Time taken to deliver notifications in seconds",
            unit="s"
        )
        
        # Channel-specific metrics
        self.notifications_by_channel = self._meter.create_counter(
            name="notifications.by.channel.total",
            description="Total notifications sent by channel",
            unit="1"
        )
        
        self.channel_delivery_time = self._meter.create_histogram(
            name="notification.channel.delivery.time",
            description="Delivery time by channel in seconds",
            unit="s"
        )
        
        self.channel_failures = self._meter.create_counter(
            name="notification.channel.failures.total",
            description="Total failures by channel",
            unit="1"
        )
        ...
        self.notification_status_changes = self._meter.create_counter(
            name="notification.status.changes.total",
            description="Total notification status changes",
            unit="1"
        )
        ...
        self.notifications_queued = self._meter.create_up_down_counter(
            ...
            description="Number of queued notifications",
            unit="1"
        )
        ...
        self.notifications_read = self._meter.create_counter(
            ...
            description="Total notifications read by users",
            unit="1"
        )
        
        self.notifications_clicked = self._meter.create_counter(
            name="notifications.clicked.total",
            description="Total notifications clicked by users",
            unit="1"
        )
        
        self.time_to_read = self._meter.create_histogram(
            name="notification.time.to.read",
            description="Time between notification sent and read in seconds",
            unit="s"
        )
        
        self.unread_count = self._meter.create_up_down_counter(
            name="notifications.unread.count",
            description="Current unread notifications per user",
            unit="1"
        )
        
        # Throttling metrics
        self.notifications_throttled = self._meter.create_counter(
            name="notifications.throttled.total",
            description="Total notifications throttled",
            unit="1"
        )
        ...
        self.notification_retries = self._meter.create_counter(
            name="notification.retries.total",
            description="Total notification retry attempts",
            unit="1"
        )
        
        self.retry_success_rate = self._meter.create_histogram(
            name="notification.retry.success.rate",
            description="Success rate of retried notifications",
            unit="%"
        )
        
        # Batch processing metrics
        self.batch_notifications_processed = self._meter.create_counter(
            name="notification.batch.processed.total",
            description="Total notifications processed in batches",
            unit="1"
        )
        ...
        self.template_render_errors = self._meter.create_counter(
            name="notification.template.render.errors.total",
            description="Total template rendering errors",
            unit="1"
        )
        ...
        self.webhook_response_status = self._meter.create_counter(
            name="notification.webhook.response.status.total",
            description="Webhook response status codes",
            unit="1"
        )
        ...
        self.slack_api_errors = self._meter.create_counter(
            name="notification.slack.api.errors.total",
            description="Total Slack API errors",
            unit="1"
        )
        ...
        self.subscription_changes = self._meter.create_counter(
            name="notification.subscription.changes.total",
    ...
    def record_notification_delivery_time(self, duration_seconds: float, notification_type: str,
    ...
    def record_notification_status_change(self, notification_id: str, from_status: str, to_status: str) -> None:
        self.notification_status_changes.add(
            1,
            attributes={
    ...
    def record_template_render(self, duration_seconds: float, template_name: str, success: bool) -> None:
        ...
        if not success:
            self.template_render_errors.add(
                1,
    ...
    def record_webhook_delivery(self, duration_seconds: float, status_code: int, url_pattern: str) -> None:
        ...
        self.webhook_response_status.add(
            1,
            attributes={
                "status_code": str(status_code),
                "url_pattern": url_pattern
    ...
    def record_slack_delivery(self, duration_seconds: float, channel: str, success: bool,
                             error_type: str | None = None) -> None:
        self.slack_delivery_time.record(
            duration_seconds,
            attributes={
                "channel": channel,
                "success": str(success)
