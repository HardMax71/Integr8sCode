...
class DLQMetrics(BaseMetrics):
    ...
    def _create_instruments(self) -> None:
        ...
        self.dlq_messages_received = self._meter.create_counter(
            name="dlq.messages.received.total",
            description="Total number of messages received in DLQ",
            unit="1"
        )

        self.dlq_messages_retried = self._meter.create_counter(
            name="dlq.messages.retried.total",
            description="Total number of DLQ messages retried",
            unit="1"
        )

        self.dlq_messages_discarded = self._meter.create_counter(
            name="dlq.messages.discarded.total",
            description="Total number of DLQ messages discarded",
            unit="1"
        )

        # DLQ processing metrics
        self.dlq_processing_duration = self._meter.create_histogram(
            name="dlq.processing.duration",
            description="Time spent processing DLQ messages in seconds",
            unit="s"
        )

        self.dlq_message_age = self._meter.create_histogram(
            name="dlq.message.age",
            description="Age of messages in DLQ in seconds",
            unit="s"
        )
        ...
        self.dlq_queue_size = self._meter.create_up_down_counter(
            name="dlq.queue.size",
            description="Current size of DLQ by topic",
            unit="1"
        )
        ...
        self.dlq_processing_errors = self._meter.create_counter(
            name="dlq.processing.errors.total",
    ...
    def record_dlq_message_received(self, original_topic: str, event_type: str) -> None:
        self.dlq_messages_received.add(
            1,
            attributes={
    ...
    def record_dlq_processing_duration(self, duration_seconds: float, operation: str) -> None:
        self.dlq_processing_duration.record(
            duration_seconds,
    ...
    def update_dlq_queue_size(self, original_topic: str, size: int) -> None:
        ...
        if delta != 0:
            self.dlq_queue_size.add(
                delta,
                attributes={"original_topic": original_topic}
            )
    ...
    def record_dlq_throughput(self, messages_per_second: float, original_topic: str) -> None:
        self.dlq_throughput_rate.record(
            messages_per_second,
            attributes={"original_topic": original_topic}
        )
