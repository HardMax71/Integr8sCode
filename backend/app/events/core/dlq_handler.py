...
def create_dlq_error_handler(
    ...
    async def handle_error_with_dlq(error: Exception, event: BaseEvent) -> None:
        ...
        logger.error(
            f"Error processing event {event_id} ({event.event_type}): {error}. Retry {retry_count + 1}/{max_retries}",
            exc_info=True,
        )

        # Send to DLQ if we've exceeded max retries
        if retry_count >= max_retries:
            logger.warning(f"Event {event_id} exceeded max retries ({max_retries}). Sending to DLQ.")
...
def create_immediate_dlq_handler(
    ...
    async def handle_error_immediate_dlq(error: Exception, event: BaseEvent) -> None:
        ...
        await producer.send_to_dlq(original_event=event, original_topic=original_topic, error=error, retry_count=0)
