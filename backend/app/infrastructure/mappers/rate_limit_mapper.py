...
class RateLimitConfigMapper:
    ...
    def to_dict(config: RateLimitConfig) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_overrides": {uid: user_mapper.to_dict(user_limit)
                              for uid, user_limit in config.user_overrides.items()},
    ...
    def from_dict(data: Dict[str, Any]) -> RateLimitConfig:
        ...
        return RateLimitConfig(
            default_rules=[rule_mapper.from_dict(rule_data)
                          for rule_data in data.get("default_rules", [])],
            user_overrides={uid: user_mapper.from_dict(user_data)
                           for uid, user_data in data.get("user_overrides", {}).items()},
