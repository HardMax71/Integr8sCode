...
# Calculate the actual project root based on conftest.py's location
# __file__ -> backend/tests/conftest.py
# .parent -> backend/tests/
# .parent.parent -> backend/
# .parent.parent.parent -> ACTUAL PROJECT ROOT
ACTUAL_PROJECT_ROOT = pathlib.Path(__file__).parent.parent.parent

# Path to CA cert relative to the *actual* project root
CA_CERT_PATH = ACTUAL_PROJECT_ROOT / "shared_ca" / "rootCA.pem"

# Path to .env.test relative to *this* conftest file (backend/tests/.env.test)
...
async def client() -> AsyncGenerator[httpx.AsyncClient, None]:
    ...
    if not CA_CERT_PATH.is_file():
        pytest.fail(
            ...
            f"Ensure the cert-generator service ran and the shared_ca directory/volume "
            f"exists at the project root.",
            pytrace=False
        )
    else:
...
async def db() -> AsyncGenerator[AsyncIOMotorDatabase, None]:
    ...
    if not settings.MONGODB_URL or not settings.PROJECT_NAME:
        pytest.fail(f"MONGODB_URL or PROJECT_NAME missing after loading settings from {settings_env_file}. "
                    f"Loaded URL: '{settings.MONGODB_URL}', Project: '{settings.PROJECT_NAME}'")

    if "localhost:27017" not in settings.MONGODB_URL and "mongo:27017" not in settings.MONGODB_URL:
        pass
